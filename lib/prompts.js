...
LOGO & LABEL PRESERVATION (mandatory):
- Any visible brand logo, label, tag, or woven label must remain EXACTLY as shown in the reference:
  same position, same size, same orientation, same text content, same stitching style.
- If a care label or size tag is visible, preserve its text, font, and attachment method.
- Embroidered logos or monograms must retain their exact thread colors, stitch pattern, and placement.
- Do NOT remove, add, relocate, resize, or alter any branding element.`.trim();
...
${OUTPUT_QUALITY}
    ...
    // Images: [validated product packshot]
    // Source: Adapted from 3x3 grid prompt → individual angles for e-commerce textile (Claid AI guide)
    alternateAngle: (angleType, detailFocus, notes) => {
        const ANGLE_INSTRUCTIONS = {
            ...
            "dos": "Generate a perfectly straight BACK VIEW of this exact product, camera perpendicular to the back, showing rear construction, labels, and closures.",
...
INSTRUCTION:
Using this image as the ABSOLUTE product identity reference, ${instruction}

PRODUCT IDENTITY (non-negotiable):
- This is the SAME product as in the reference — not a similar one, not a recreation.
  Every color, pattern, material, hardware detail, label, and construction element must
  match the reference EXACTLY.
- If the reference shows a pattern/print, it must appear consistently across all visible
...
- The product must occupy the same approximate proportion of the frame as the reference.
