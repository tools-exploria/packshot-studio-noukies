...
INTEGRATION INTO THE SCENE (critical — avoid the "photoshop cutout" look):
The product comes from a separate studio packshot with flat, neutral lighting on a
white background. You MUST re-light it to match the scene, NOT paste it as-is.
- The product takes on the SCENE's lighting direction, colour temperature, and
  shadow direction. It must look photographed in this exact scene, not extracted
  from a separate shoot and dropped in.
- Reproduce the scene's KEY LIGHT angle and intensity on the product surfaces. If
  the scene has golden hour backlight from window-left, the product receives that
  same backlight from window-left.
- Cast a natural contact shadow under or behind the product, matching the scene's
  main light source direction and softness.
- Subtle colour bleed from surrounding scene elements onto the product (warm wood
  reflects warm tones onto fabric, blue walls reflect cool tones onto plush, etc.).
- Matching grain, noise level, and depth-of-field across product and scene — the
  product must NOT look sharper, cleaner, or more "studio" than the scene around it.
${productCount === 1 ? "- SINGLE-PRODUCT INTEGRATION: with only one product visible, you have no other product anchor sharing the scene's lighting context. Pay EXTRA attention to its individual lighting consistency — fully re-light the packshot's flat studio lighting to belong to the scene's ambient lighting. A single product that doesn't blend is far more obvious than one of several products.\n" : ""}
${withNotes(notes)}
OUTPUT:
A finished lifestyle photo, editorial baby brand campaign quality, indistinguishable
from a real photograph taken in this scene. The product${productCount > 1 ? "s feel" : " feels"}
intentionally photographed in the scene, not composited on top.
`,
