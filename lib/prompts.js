...
INSTRUCTION:
The first image is a scene inspiration reference. The other ${productCount} image${productCount > 1 ? "s are" : " is"} the
Noukies product${productCount > 1 ? "s" : ""} to integrate. Generate a finished lifestyle photo
where the product${productCount > 1 ? "s are" : " is"} placed naturally in a scene inspired by the reference.

SCENE:
- Borrow from the reference: mood, palette, lighting direction, material vocabulary,
  decor logic. The reference's composition is a starting point — adapt framing,
  perspective, and element arrangement freely to best showcase the product${productCount > 1 ? "s" : ""}.
- Follow the Noukies brand: soft pastels (cream #F5F0E8, sage #B5C9A8, powder pink,
  warm grey, lavender, sky blue), soft diffused warm light, Bonpoint / Petit Bateau
  editorial aesthetic. Never saturated, never harsh.${sceneTweaks && sceneTweaks.trim() ? `\n- User modifications to apply: ${sceneTweaks.trim()}` : ""}

PRODUCT FIDELITY (preserve EXACTLY from each product's reference image):
- Every label, every tag, every CE marking, every illustration, every stitching line,
  every ribbon, every snap, every plush appliqué, every hardware element — same
  position, same orientation, same color, same material.
- Same exact colours, pattern/print, fabric texture, material, proportions.
- Sharp focus on material texture and construction details.
- A white fabric stays pure white; coloured fabric keeps its exact shade.
- Do NOT invent, modify, recolour, simplify, or relocate any product element.

PLACEMENT (EXACTLY ${productCount} product${productCount > 1 ? "s" : ""} visible — no more, no less):
- Each product is placed where ITS OWN textual description in the manifest says.
  ${productCount > 1 ? `Read each product description carefully — that's where placement intent lives.` : `The product description in the manifest may include placement intent — follow it.`}
- If no placement is stated for a product, place it where it makes visual and logical
  sense (on furniture, in a crib, draped, sitting).
- Every product clearly visible and identifiable. No product hidden or obscured.
${productCount > 1 ? "- Distribute across the frame; do NOT pile them together.\n" : ""}- Do NOT invent additional products that aren't in the references.

INTEGRATION (critical — avoid the photoshop-cutout look):
The product${productCount > 1 ? "s come" : " comes"} from studio packshot${productCount > 1 ? "s" : ""} with flat lighting on white. RE-LIGHT them to belong to the scene:
- Adopt the scene's key-light angle, colour temperature, and ambient tone on the
  product surfaces (e.g. golden hour backlight from window-left → product gets that
  same backlight from window-left).
- Cast a natural contact shadow under or behind each product, matching the scene's
  main light direction and softness.
- Subtle colour bleed from surrounding scene elements onto the product (warm wood
  reflects warm tones onto fabric, cool walls reflect cool tones, etc.).
- Match the scene's grain, noise level, and depth-of-field — no "sharper than the
  scene" cutout artifact.${productCount === 1 ? "\n- With only one product, no other anchor shares the lighting context — pay extra attention to making this single product belong to the scene's atmosphere." : ""}
${withNotes(notes)}
OUTPUT:
A finished lifestyle photo, editorial baby brand campaign quality, indistinguishable
from a real photograph taken in this scene. No composited or pasted-on appearance.
`,
    ...
    chromaKeyBg: (hex, colorName) =>
        `Replace the background of this image with a UNIFORM SOLID COLOUR FILL: pure ${hex} (${colorName}).

The background is ONE SINGLE RGB VALUE at every pixel — like a paint-bucket fill in Photoshop, like a real-world solid-colour studio backdrop (a single roll of cyclorama paper).

ABSOLUTELY NOT ALLOWED in the background (these are common training-data artifacts you must avoid):
- NO checker pattern, NO chess pattern, NO grid of alternating squares.
- NO transparency grid (the Photoshop / GIMP transparency indicator pattern — your training data has many examples of this in green, but they are WRONG here, we want a true solid fill).
- NO pattern of any kind, no texture, no noise.
- NO gradient, no light falloff, no vignette, no variation in shade.
- NO shadows, NO reflections, NO bokeh.

Every single background pixel must be EXACTLY ${hex}. Imagine pouring a paint bucket of ${colorName} onto every non-product area — that's the target.

Keep the product EXACTLY as-is: same shape, colours, materials, angle, hardware, labels, stitching.`,
};
