...
- Maintain realistic fabric texture and weave visible in the original — the surface should
  NOT look flat, painted, or digitally filled. It must read as real printed textile.
...
PRODUCT IDENTITY (non-negotiable):
- The product must be PIXEL-PERFECT to the identity reference: same exact colours,
  same exact pattern/print, same exact fabric texture and material, same proportions,
  same hardware.
- Mentally inventory EVERY visible element on the identity reference before generating:
  labels, woven brand tags, CE markings, illustration tags, decorative corner tabs,
  stitching lines, plush appliqués, ribbons, snaps, embroidered details. The output
  must contain EXACTLY the same inventory — no more, no less, in the same positions.
- Do NOT add, remove, simplify, relocate, or invent any label, tag, marking, tab, or
  decorative element. A white fabric stays pure white; coloured fabric keeps its
  exact shade.
- Hidden, folded, or partially visible elements stay hidden, folded, or partially
  visible in the output. Never reconstruct a face, character, or detail you cannot
  clearly see in the reference.
- PRODUCT NOTES (if present) name elements but never authorize reconstruction. If
  notes say "raton laveur" and the raccoon is folded in the reference, it stays folded.
...
- When a SKETCH and a PHOTO are both provided as bases: structure from the sketch,
  visual identity (colour, pattern, material, every label and tag) from the photo.
  Sketches never contribute colour or material.
...
- Preserve the EXACT shape and construction: sleeve length, collar shape, cut,
  proportions, seams, and every structural detail must match the reference.
  Do NOT re-interpret the garment's cut when folding it.
- Do NOT shift colours, do NOT warm/yellow whites, do NOT change fabric texture.
- Do NOT add or remove buttons, zippers, snaps, labels, tags, or stitching.
...
`,
    ...
    // Régénère une scène (chambre, mood board…) à partir d'une image de référence,
    // appliquant les retouches utilisateur. La sortie est UNE scène (pas de produit).
    sceneFromReference: (tweaks, applyBrand, notes) => `
${applyBrand ? BRAND_BRIEF + "\n\n" : ""}TASK: Regenerate the reference scene shown in the attached image, applying the user's
adjustments below. The output is a STANDALONE SCENE — no product, no person unless the
reference includes one and the user keeps them.

REFERENCE INTERPRETATION:
- The attached image defines the base composition, layout, furniture placement, and overall
  mood you must preserve unless the user explicitly asks to change them.
- Preserve the geometry of the room (walls, floor, windows, furniture positions) unless the
  tweaks below override.
- The reference may be a Pinterest mood board, a photo, a render — treat it as a creative
  brief, not a literal target.

USER ADJUSTMENTS:
${tweaks.trim() ? tweaks.trim() : "(none — produce a refined, polished version of the reference scene with no specific changes)"}

${applyBrand ? `BRAND COHERENCE (Noukies):
- Soft pastels palette (cream #F5F0E8, sage #B5C9A8, powder pink #F0D4D8, warm grey #C4BAB0).
- Soft diffused natural light, warm golden tone, no harsh shadows.
- Premium tactile materials (organic cotton, oak, linen). Nothing synthetic-looking.
- Calm, reassuring mood — Bonpoint/Petit Bateau editorial aesthetic.\n` : ""}${withNotes(notes)}
OUTPUT:
Photorealistic interior photography, editorial quality, balanced composition, warm natural tones.
Kodak Portra 400 color science when applicable. The scene must look like a finished location,
ready to host a product photoshoot.
    `,
    ...
    //
    // S'inspire de la scène SANS la figer. Le modèle peut ajuster cadrage/lumière/angle
    // pour accommoder naturellement les produits.
    productsInScene: (productCount, notes) => `
INSTRUCTION:
The first image is a SCENE REFERENCE — a beautifully styled environment you must use as
creative inspiration. The other ${productCount} image${productCount > 1 ? "s are" : " is"} the
Noukies product${productCount > 1 ? "s" : ""} to integrate naturally into a scene inspired
by that reference.

${BRAND_BRIEF}

SCENE INSPIRATION (not a literal copy):
- The reference scene gives you the MOOD, PALETTE, LIGHTING DIRECTION, and OVERALL ATMOSPHERE.
- Adapt the framing, angle, and composition as needed so the product${productCount > 1 ? "s feel" : " feels"}
  natural and well-presented. The scene is a starting point, not a constraint.
- Borrow furniture style, materials, decor logic from the reference — but feel free to add,
  remove, or rearrange elements to better serve the product${productCount > 1 ? "s" : ""}.
- You may zoom in, change the camera angle, or shift the perspective from the reference.

PRODUCT PLACEMENT:
- Place ${productCount === 1 ? "the product" : `all ${productCount} products`} naturally
  in the scene — on furniture, draped, sitting, placed where it makes visual and logical sense.
- Every product must be clearly visible and identifiable. No product hidden or obscured.
${productCount > 1 ? "- Distribute the products across the frame — do NOT pile them together.\n" : ""}
PRODUCT FIDELITY (non-negotiable):
- Each product must appear EXACTLY as in its reference: same colours, pattern, materials,
  shape, proportions, hardware. Do NOT modify, recolour, or simplify any product.
- EXACTLY ${productCount} product${productCount > 1 ? "s" : ""} visible — no more, no less.
- Do NOT invent products not in the references.
${withNotes(notes)}
OUTPUT:
Photorealistic interior or lifestyle photography, editorial baby brand campaign quality.
Warm natural tones, balanced depth of field with products in focus. The image should feel
like a high-end catalogue spread — products feel intentionally placed, not photoshopped in.
`,
