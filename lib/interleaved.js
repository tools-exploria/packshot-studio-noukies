...
export const IMAGE_ROLES = {
    ...
    existingProduct: {
        label: "REFERENCE COMPLEMENTAIRE",
        labelEn: "COMPLEMENTARY REFERENCE",
        extract: "ONLY what the user description specifies (e.g., a hidden angle, a sharp detail close-up, a material texture, a specific element on another product). Read the user description carefully.",
        ignore: "everything NOT mentioned in the user description — do NOT borrow shape, silhouette, structure, or any element the description does not name. This reference enriches the base; it never overrides it.",
    },
    roomProduct: {
    ...
    // Variante de `scene` pour les outils où la scène est une INSPIRATION
    // (palette, lumière, mood) et NON un canvas figé. On ne demande
    // explicitement PAS d'extraire la "composition" pour éviter la
    // contradiction avec un prompt qui demande au modèle d'adapter le cadrage
    // aux produits.
    sceneInspiration: {
        label: "REFERENCE D'INSPIRATION (scène)",
        labelEn: "SCENE INSPIRATION REFERENCE",
        extract: "mood, palette, lighting direction, material vocabulary, decor logic",
        ignore: "composition and framing — these will be adapted around the products. Do not copy the exact layout of the reference.",
    },
    // Variante de `roomProduct` pour les outils où la scène n'est pas
    // forcément une chambre (peut être salon, terrasse, extérieur, etc.).
    productInScene: {
        label: "PRODUIT A INTEGRER DANS LA SCENE",
        labelEn: "PRODUCT TO INTEGRATE INTO THE SCENE",
        extract: "full product identity: shape, colour, pattern, material, hardware, labels, proportions, every visual detail. The image's textual description may also include WHERE this product should be placed in the scene.",
        ignore: null,
    },
};
